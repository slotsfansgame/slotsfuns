<?php
//добавляем текстовый редактор в админку (бэкэнд)
...
function wpm_taxonomy_edit_meta_field($term) {
 ...
 $term_meta = get_option( "taxonomy_$t_id" );
  ...
  ?>
  <tr class="form-field">
  <th scope="row" valign="top"><label for="term_meta[custom_term_meta]">Этот текст появится после списка товаров</label></th>
    <td>
...
//Добавим вывод из редактора в шаблон на сайт (фронтэнд)
...
function theme_sort_change( $translated_text, $text, $domain ) {
    ...
    if ( is_woocommerce() ) {
        ...
        switch ( $translated_text ) {
            ...
            case 'Исходная сортировка' :

                $translated_text = __( 'По умолчанию', 'theme_text_domain' );
				break;
            
			case 'Сортировка по более позднему' :

                $translated_text = __( 'По новизне обзоров', 'theme_text_domain' );
                break;
				
			case 'Вас может заинтересовать' :

                $translated_text = __( 'Похожие обзоры', 'theme_text_domain' );
                break;
				
			case 'Детали' :

                $translated_text = __( 'Характеристики', 'theme_text_domain' );
                break;
        }
